...
#### (가) 개요
1. WEP 인증 상의 문제점을 해결하기 위해 802.1x 표준에서는 EAP 인증기능을 제공하고 있다. EAP(Extensible Authentication Protocol) 프로토콜은 RFC 2284에 공식적인 명세가 발표되었으며 무선랜 표준인 IEEE 802.1x에서 사용자 인증방법으로 사용되고 있다.
2. EAP 프로토콜은 어떤 링크에도 접속이 가능한 단순한 캡슐화 개념의 프로토콜로 EAP 자체는 실제 인증 프로토콜이 아니지만 다양한 종류의 인증 방법을 전송하는 역할을 한다.
   * EAP는 내부적으로 EAP-MD5, 전송 계층 보안(EAP-TLS), 터널 방식으로 개량된 TLS(EAP-TTLS), PEAP 등의 다양한 인증 방법을 지원한다.

#### (나) 동작방식
1. 무선랜 클라이언트인 사용자 A가 AP에 무선랜 접속을 요청한다.
2. 무선AP는 사용자 A가 연결을 설정할 때까지 다른 사용자의 연결 시도를 차단하고 사용자 A로 연결 요청에 대한 응답메시지를 보낸다.
3. 사용자 A는 EAPoL-시작 메시지를 AP에 보내 RADIUS를 이용한 연결설정을 시작할 것을 AP에 알린다.
4. 무선AP는 사용자가 보낸 EAPoL-시작 메시지를 받은 후 EAP-요청 메시지를 사용자 A로 전송하여 사용자 A의 정보를 요구한다.
5. 사용자 A는 네트워크에 로그온하기 위해 사용자명과 암호를 입력창을 통해 입력하여 사용자 인증을 위한 정보를 무선AP에 보낸다.
6. 무선AP는 사용자에게 받은 정보를 RADIUS 서버에 접속요청 메시지에 담아 상호 인증을 요구한다.
7. RADIUS 서버는 AP에게 사용자 A로 전송할 사용자 인증 챌린지 메시지를 보낸다.
8. 무선AP는 RADIUS 서버에서 보낸 사용자 인증 챌린지를 사용자 A로 전송한다.
9. 사용자 A는 챌린지에 대한 응답으로서 사용자명과 암호를 해시하여 EAP 응답메시지를 구성하여 무선AP에 전송한다.
10. 무선AP는 사용자 A로부터 받은 정보를 이용하여 RADIUS 서버에 Radius-접속요청 메시지를 보낸다.
11. 사용자 관리 DB 정보를 사용하여 RADIUS 서버는 자신이 보낸 챌린지에 대한 대응 메시지를 생성하여, 사용자 A가 보내온 응답메시지의 값과 비교하여 사용자 A를 인증한다.
12. 사용자 A와 RADIUS 서버의 상호 인증이 성공적으로 완료된다.

## (7) 기타 무선랜 접속 인증 기술
### 1) SSID 설정을 통한 접속 제한
1. 무선랜 관리자가 무선AP의 SSID를 브로드캐스트하지 않도록 설정(숨김 모드)하여 SSID를 모르는 사용자는 무선랜에 접속할 수 없게 만드는 인증방식을 말한다.
2. 무선 데이터 분석 도구(e.g. airodump)를 이용한 무선 데이터 수집, 분석을 통해 손쉽게 SSID를 알아낼 수 있는 단점이 있다.

### 2) MAC 주소 인증(MAC 주소 필터링)
1. 무선랜 접속을 허용할 단말의 MAC 주소를 무선AP에 사전 등록하여 등록된 단말에 대해서만 접속을 허용해주는 인증방식을 말한다.
2. 설정 방법이 간단하고 기본적인 공격을 효과적으로 방어할 수 있는 인증 기술로 많은 종류의 무선AP에서 지원하는 기법이지만 공격자가 정상 단말의 MAC 주소로 위조함으로써 쉽게 무력화될 수 있다.
3. 공격자는 인증된 단말과 무선AP 사이의 패킷을 분석하여 단말의 MAC 주소를 알아낸 후 공격자의 MAC 주소를 인증된 단말의 MAC 주소로 위조하여 접속함으로써 MAC 필터링을 무력화시킬 수 있다.
4. MAC 주소 인증을 수행하는 무선AP에 대한 관리 소홀 또는 해킹으로 인해 등록된 MAC 주소 정보가 외부로 노출될 수 있는 취약점이 있고 대규모의 네트워크에서 MAC 주소를 관리하기 위한 업무 부담이 크다.

## (8) WPA-PSK 접속/인증 패스워드 크래킹 실습
### 1) 실습환경
#### (가) 실습도구
1. OS : 칼리 리눅스
2. aircrack-ng : 무선랜에 대한 패킷 모니터링, 크래킹, 분석 도구 등이 탑재된 소프트웨어

#### (나) 대상 무선AP 설정 확인
1. 무선 AP 주요 설정 내용을 살펴보면 다음과 같다.
   * 네트워크 이름(SSID) : ALGISA_AP
   * 인증기술/암호기술 : WPA의 PSK 인증기술과 TKIP 암호기술 사용
   * 네트워크 키(PSK) : algisa102
  
### 2) 무선랜 인터페이스 모니터 모드 활성화
#### (가) iwconfig 명령을 통해 무선랜 인터페이스 상태 확인
1. iwconfig 명령은 ifconfig와 유사한 명령으로 무선랜 인터페이스 정보를 확인하거나 환경 설정을 위해 사용하는 명령어이다. 명령어 결과를 살펴보면 wlan0 무선랜 인터페이스가 설정되어있는 것을 확인할 수 있다.

#### (나) 무선랜 인터페이스 모니터 모드 활성화
1. ```airmon-ng start wlan0``` 명령을 통해 wlan0 인터페이스의 동작 모드를 모니터 모드로 전환한다.
2. 무선랜 인터페이스의 모니터 모드란 유선랜 인터페이스의 무차별모드와 동일하게 해당 인터페이스로 전송되지 않은 주변의 모든 무선 패킷을 수신할 수 있는 모드를 말한다.
   * 무선랜 인터페이스의 기본 동작 모든느 관리 모드(Managed Mode)로 자신에게 전송되는 무선 패킷만을 수신하도록 동작한다.

### 3) WPA-PSK 인증 패스워드 크래킹
#### (가) 주변 무선 AP 및 단말들의 네트워크 정보를 수집하여 해킹할 무선AP 및 단말을 선정한다.
1. ```airodump-ng wlan0mon``` 명령을 통해 모니터 모드로 동작중인 wlan0mon 인터페이스로 주변 무선 패킷을 수집한 후 패스워드를 크랙할 대상 AP와 단말을 선택한다.
2. 대상 무선AP와 단말정보를 살펴보면 다음과 같다.
   * 네트워크 이름(SSID) : ALGISA_AP
   * 무선AP MAC 주소(BSSID) : 00\:08\:9F\:6A\:48\:CE
   * 보안 표준(ENC, CIPHER, AUTH) : WPA 보안 규격의 TKIP 암호기술과 PSK 인증기술 사용
   * 대상 무선AP에 연결된 단말 MAC 주소 : AC\:EE\:9E\:DE\:6A\:2F

#### (나) 해킹할 무선AP와 단말 사이의 패킷을 수집한다.
1. ```airodump-ng --bssid 00:08:9F:6A:48:CE -c 5 -w wpa wlan0mon``` 명령을 실행하여 wlan0mon 무선랜 인터페이스를 통해 MAC 주소가 00\:08\:9F\:6A\:48\:CE인 무선AP의 5번 채널로 통신하는 무선 패킷을 수집하여 wpa 접두어(Prefix)를 가진 덤프파일로 저장한다.
2. 명령어 옵션 설명
   * ```--bssid 00:08:9F:6A:48:CE``` : 대상 무선AP의 BSSID(AP의 MAC 주소)
   * ```-c 5``` : 대상 무선AP의 채널
   * ```-w wpa``` : 패킷 덤프파일의 접두어(Prefix)
   * ```wlan0mon``` : 패킷을 수집할 모니터 모드의 무선랜 인터페이스 명
3. 명령을 수행하면 위와 같이 wpa 접두사로 시작하는 다양한 덤프파일이 생성된 것을 확인할 수 있다.

#### (다) 무선AP와 단말간 재인증 발생하도록 공격한다.
1. 대상 무선AP에 연결되어있는 단말에 인증해제 패킷을 전송하여 무선AP와 단말 간 연결을 강제로 종료시키면 단말은 재인증을 시도하기 때문에 패스워드 크랙을 위해 필요한 4-way handshake 과정의 인증 수행 패킷을 수집할 수 있다.
2. ```aireplay-ng --deauth 100 -a 00:08:9F:6A:48:CE -c AC:EE:9E:DE:6A:2F wlan0mon``` 명령을 실행하여 wlan0mon 무선랜 인터페이스를 통해 MAC 주소가 00\:08\:9F\:6A\:48\:CE인 무선AP와 MAC 주소가 AC\:EE\:9E\:DE\:6A\:2F인 단말간의 연결을 종료하는 인증해제 패킷을 100회 전송한다.
3. 명령어 옵션 설명
   * ```--deauth 100``` : 인증해제 패킷 전송 횟수를 의미한다. "-0 횟수" 형식으로 표현할 수도 있다.
   * ```-a 00:08:9F:6A:48:CE``` : 대상 무선AP의 BSSID(AP의 MAC주소)
   * ```-c AC:EE:9E:DE:6A:2F``` : 대상 단말의 MAC 주소
   * ```wlan0mon``` : 인증해제 패킷을 전송할 무선랜 인터페이스 명

#### (라) 수집한 인증패킷 덤프파일을 이용하여 패스워드 크랙 수행
1. ```aircrack-ng -w ./DIC/pw.lst wpa-01.cap``` 명령을 실행하여 wpa-01.cap 덤프파일과 pw.lst 사전 파일을 이용하여 사전 공격을 통해 패스워드를 크랙한다. 패스워드 크랙 성공 시 KEY FOUND! 메시지와 함께 크랙한 패스워드를 출력한다.
2. 명령어 옵션 설명
   * ```-W ./DIC/pw.lst``` : 사전 공격을 위한 사전 파일
   * ```wpa-01.cap``` : airodump-ng 명령을 통해 수집한 인증패킷을 담고 있는 덤프 파일

### 4) 크랙한 패스워드를 이용한 무선전송 데이터 복호화
1. ```airdecap-ng -p "algisa102" -e ALGISA_AP wpa-01.cap``` 명령을 실행하여 wpa-01.cap 덤프파일에 있는 암호화된 패킷을 복호화한다.
2. 명령어 옵션 설명
3. ```-p "algisa102"``` : 크랙한 패스워드
4. ```-e ALGISA_AP``` : 대상 무선 네트워크의 SSID
5. ```wpa-01.cap``` : 암호화된 무선전송 데이터를 담고 있는 덤프파일

## (9) 무선랜 보안대책
### 1) 기업 네트워크에서의 무선랜 보안
1. 무선 단말 인증
   * 기업 네트워크 환경에서 무선단말 인증은 WPA2-기업의 RADIUS 서버를 활용한 EAP 기반 인증을 하도록 권고한다.
2. 전송 데이터 암호화
   * 전송 데이터 암호화는 안전성이 검증된 WPA2 보안규격의 AES-CCMP 방식을 권고한다.
  
### 2) 개인 및 SOHO 사업자의 무선랜 보안
1. 사설망을 통한 인증 및 암호화
   * 개인 및 SOHO 환경은 인증서버를 두기 어려운 상황이 대부분이므로 WPA2-개인의 PSK 인증방식을 설정한 후 패스워드 크랙에 충분히 안전한 인증 패스워드를 이용한다.
   * 전송 데이터 암호화는 안전성이 검증된 WPA2 보안규격의 AES-CCMP 방식을 권장한다.
2. 무선랜 사업자의 서비스 이용
   * 무선랜 사업자에서 제공하는 인증서버를 통한 단말 인증기능을 사용하는 방법도 고려해볼 수 있다.
  
### 3) 무선AP 물리적/관리적 보안
1. SSID 변경 및 브로드캐스트 금지
   * SSID를 공개된 초기 설정값이 아닌 새로운 값으로 변경하여 공격자가 쉽게 인지하지 못하도록 한다.
   * SSID를 브로드캐스트하지 않고 숨김 모드로 설정하여 공격자에게 무선랜이 노출되는 것을 최소화한다.
2. 무선AP의 Default Password 변경
   * 무선AP에 설정된 초기 패스워드는 공개되어 있어 타인에 의해 쉽게 접속할 수 있으므로 반드시 변경 후 사용해야 한다.
   * 무선AP 패스워드는 추측하기 어려운 안전한 생성 방법에 따라 생성하고 주기적으로 변경하여 공격자가 무선AP 관리자 모드로 접속하는 것을 방지한다.
3. 무선AP의 물리적 접근 제한
   * 무선AP의 도난 및 공격자의 접근으로부터 보호할 수 있도록 보호 케이스 설치, AP 리셋 버튼 차단 등 물리적 보안대책 확보한다.
  
4. 정리하면 인증은 WPA의 RADIUS 서버를 활용한 EAP 사용을 권고하고 암호화는 WPA의 AES-CCMP 방식을 권고한다. SSID 변경 및 브로드캐스트를 금지하고 무선AP의 Default Password 변경하고 무선AP의 물리적 접근 제한하기
