...
3. 정리하면 BCP는 재난에도 정상적인 운영 가능하도록 데이터 백업 및 복구 뿐만 아니라 서비스 지속성 보장하는 문서
...
3. 정리하면 재난 복구 계획(DRP)은 재난 후 대체 사이트에서의 목표 시스템, 응용프로그램 등의 재개 계획
...
3. 정리하면 BCP는 사업활동이나 프로세스가 중단되는 것에 대응, DRP는 핵심 정보시스템, 데이터가 중단되는 것에 대응
...
3. 정리하면 BIA는 모든 재해 고려하고 잠재적인 손실 추정, 재난 우선순위 분류류
...
7. 정리하면 BIA의 주요 활동은 데이터 수집, 다이어그램 작성, 문서화 및 비즈니스 체계 개발, 필요한 자원 식별, 최대 허용 중단 시간(MTD) 산정, 우선순위 결정하는 것
...
3. 정리하면 위험분석은 위험을 식별하고 안화하거나 감소시키는 비용 관리 프로세스
...
4. 정리하면 미러 사이트는 주 센터 = 원격지, 실시간 동시 서비스 방식, 복구 소요 시간(RTO) = 0, 
...
3. 정리하면 핫 사이트는 주 센터와 동일 수준 시스템을 재해복구센터에 대기상태로 두고 실시간 복제를 하여 최신 데이터 상태 유지하다 재해 시 활성화 시켜 복구하는 방식, 비용 비쌈
...
3. 정리하면 웜 사이트는 중요성이 높은 정보기술자원만 부분적 보유 방식, 몇 시간~ 며칠

#### (라) 콜드 사이트
1. 데이터만 원격지에 보관하고 서비스를 위한 정보 자원은 확보하지 않거나 장소 등 최소한으로만 확보하고 있다가 재해 시에 데이터를 근간으로 필요한 정보자원을 조달하여 복구하는 방식이다.
2. 주 센터의 데이터는 일 단위나 주 단위 주기로 원격지에 백업된다. 구축 및 유지비용이 가장 저렴하나 복구 소요시간이 매우 길고 신뢰성이 낮다.
3. 정리하면 콜드 사이트는 장소 등 최소한으로 확보하고 이싿가 재해 시에 데이터를 근간으로 필요한 정보자원 조달 복구, 몇 주 ~ 몇 달, 복구 소요시간 매우길다
...
4. 정리하면 상호 지원계약은 유사한 장비 환경을 가진 두 개 이상의 기업 간의 계약, 장점은 핫 사이트 구축 힘든 경우 대안, 단점은 운영 혼합 등으로 보안 문제제
...
3. 정리하면 RAID는 디스크 어레이를 통해 고용량, 고성능, 고가용성, 오류 검증
...
|유형|설명|
|:-:|:-|
|완전 백업|* **모든 데이터가 백업**되어 어떤 형태의 저장 매체에 저장시키는 것으로, 완전 백업 동안 아카이브 비트는 해제됨 </br>* 복구 과정은 한 단계지만 백업과 복구과정이 장시간 소요되고 많은 양의 백업매체가 필요|
|차등 백업|* 완전 백업을 차등 백업과 같이 사용하는 방법으로 **가장 최근의 완전 백업이후에 수정된 파일만 백업**하는 방식 </br>* 데이터 복구가 필요할 때 맨 먼저 완전 백업이 수행되고 그 다음에 차등백업이 수행됨 </br>* 차등 과정은 아카이브 비트 값을 변경하지 않으며, 백업 소요시간은 완전백업보다 짧고 증분 백업보다 길다.|
...
4. 정리하면 완전 백업은 속도 느림, 차등 백업은 증분 백업보다 백업 시간 길고 복구시간 짧다
